<template>
  <div class="animated fadeIn">
    ...
    <b-row>
      <b-col lg="4">
        <!--  挂号列表-->
        <registration-list @selectPatient="selectPatient">
        </registration-list>
        <group-examination :type="type" @onCite="onCite"></group-examination>
      </b-col>
      <b-col lg="8">
        ...
        <b-card header="开立检查">
          <div slot="header">
            <!-- slot设置插槽便于模板数据的精准插入-->
            <a href="#" @click.prevent="choosePatient"><i :class="openPatientFlag?'fa fa-angle-double-left fa-lg':'fa fa-angle-double-left fa-lg'" ref="switchDoor">  选择患者</i>  </a>
            <div class="card-header-actions">
              <b-button-group class="pull-right">
                <!-- 此处为清空暂存提交按钮 -->
                <b-button size="sm" :disabled="ifReadonly" @click="examinationReset" variant="danger"><i class="fa fa-undo"></i>
                  清空</b-button>
                <!-- <b-button size="sm" @click="examinationSave" class="d-sm-down-none" variant="primary"><i class="fa fa-save"></i> -->
                <!--  存档</b-button> -->
                <b-button size="sm" :disabled="ifReadonly" @click="examinationSubmit" class="d-sm-down-none" variant="success"><i
                    class="fa fa-check"></i> 提交</b-button>
              </b-button-group>
            </div>
          ...
          <b-modal ref="examination-group" size="md" @ok="onSave" okTitle="保存" cancelTitle="取消" centered title="存为模板">
            <!--  名称 -->
            <b-form-group label="模板名称" label-for="groupName" :label-cols="3" :horizontal="true">
              <b-form-input v-model="groupName" id="groupName" type="text" placeholder="请输入内容..."></b-form-input>
            </b-form-group>
            <!--  名称 -->
            <b-form-group label="模板编码" label-for="groupCode" :label-cols="3" :horizontal="true">
              <b-form-input v-model="groupCode" id="groupCode" type="text" placeholder="请输入内容..."></b-form-input>
            </b-form-group>
            <!--  范围 -->
            <b-form-group label="适用范围" label-for="groupScope" :label-cols="3">
              <b-form-radio-group id="groupScope" v-model="groupScope" :options="scopeOptions"></b-form-radio-group>
            </b-form-group>
          </b-modal>
          <b-tabs>
            <!--主模块部分的分菜单栏-->
                  ...
                  <b-col md="10">
                    <b-row>
                      <b-col align="right">
                        <b-button-group class="pull-right">
                          <!--检索处置非药品-->
                          <b-button id="searchFmedicalButton" :disabled="showAdditionDrugs" v-b-modal="computedModalId"
                            size="md" variant="outline-dark">检索</b-button>
                          <!--常用处置非药品-->
                          <b-button id="commonlyUsedFmedicalButton" :disabled="showAdditionDrugs" v-b-modal="computedCommonlyUsedModalId"
                            size="md" variant="outline-dark">常用</b-button>
                          <!--重置当前处置条目-->
                          <b-button id="resetFmedicalButton" :disabled="showAdditionDrugs" @click="resetExaminationFmedicalItem"
                            size="md" variant="outline-dark">重置</b-button>
                          <!--新增当前处置条目-->
                          <b-button id="addFmedicalButton" :disabled="showAdditionDrugs" @click="addExaminationFmedicalItem"
                            size="md" variant="outline-dark">新增</b-button>
                        </b-button-group>
                      </b-col>
                    </b-row>

                    <!--    ***********************    检索非药品的弹框     ************************       -->
                    <b-modal :id="computedModalId" okTitle="确定" cancelTitle="取消" @ok="selectFmedicalOk" @cancel="selectFmedicalCancel"
                      size="lg" centered :title="type===0?'检索检查':'检索检验'">
                      <fmedical-table @selectFmedical="selectFmedical" :type=this.type>
                      </fmedical-table>
                    </b-modal>
                    <!--   ***********************     检索常用非药品的弹框  ************************          -->
                    <b-modal :id="computedCommonlyUsedModalId" okTitle="确定" cancelTitle="取消" @ok="selectCommonlyUsedItemOk"
                      @cancel="selectCommonlyUsedItemCancel" size="lg" centered :title="type===0?'常用检查':'常用检验'">
                      <commonly-used-examination :type=2 @selectCommonlyUsedItem="selectCommonlyUsedItem"
                        :commonly-used-type=this.type :commonly-used-api=this.commonlyUsedApi :commonly-used-fields=this.commonlyUsedFields></commonly-used-examination>
                    </b-modal>
                    ...
                    <b-row>
                      <b-col md="6">
                        <!--    药品编码 drugsCode-->
                        <b-form-group label="非药品编码" label-for="fmedicalItemsCode" :label-cols="3">
                          <b-form-input id="fmedicalItemsCode" required disabled v-model="examinationFmedicalItemsForm.fmedicalItems.fmedicalItemsCode"
                            type="text" placeholder="请检索项目"></b-form-input>
                        </b-form-group>
                      </b-col>
                      <b-col md="6">
                        <!--    药品名称 drugsName-->
                        <b-form-group label="非药品名称" label-for="fmedicalItemsName" :label-cols="3">
                          <b-form-input id="fmedicalItemsName" required disabled v-model="examinationFmedicalItemsForm.fmedicalItems.fmedicalItemsName"
                            type="text" placeholder="请检索项目"></b-form-input>
                        </b-form-group>
                      </b-col>
                    </b-row>
                    ...
                    <b-row>
                      <b-col md="6">
                        <!--    药品编码 drugsCode-->
                        <b-form-group label="非药品规格" label-for="fmedicalItemsFormat" :label-cols="3">
                          <b-form-input id="fmedicalItemsFormat" required disabled v-model="examinationFmedicalItemsForm.fmedicalItems.fmedicalItemsFormat"
                            type="text" placeholder="请检索项目"></b-form-input>
                        </b-form-group>
                      </b-col>
                      <b-col md="6">
                        <!--    药品名称 drugsName-->
                        <b-form-group label="非药品价格" label-for="fmedicalItemsPrice" :label-cols="3">
                          <b-form-input id="fmedicalItemsPrice" required disabled v-model="examinationFmedicalItemsForm.fmedicalItems.fmedicalItemsPrice"
                            type="text" placeholder="请检索项目"></b-form-input>
                        </b-form-group>
                      </b-col>
                    </b-row>
                    <b-row>
                      <b-col md="6">
                        <!--    药品编码 drugsCode-->
                        <b-form-group label="开立数量" label-for="quantity" :label-cols="3">
                          <b-form-input id="quantity" required v-model="examinationFmedicalItemsForm.quantity" type="number"
                            placeholder="请添加数量"></b-form-input>
                        </b-form-group>
                      </b-col>
                      <b-col md="6">
                        <!--    药品编码 drugsCode-->
                        <b-form-group label="目的或要求" label-for="purposeRequirements" :label-cols="3">
                          <b-form-input id="purposeRequirements" required v-model="examinationFmedicalItemsForm.purposeRequirements"
                            type="text" placeholder="请输入"></b-form-input>
                        </b-form-group>
                      </b-col>
                    </b-row>
                  </b-col>
                  ...
                  <b-col md="10">
                    <b-row>
                      <b-col align="right">
                        <b-button-group class="pull-right">
                          ...
                          <b-button :disabled="showAdditionDrugs" variant="outline-dark" size="md" @click="saveExaminationFmedicalItems()">
                          </b-button>
                    ...
                    <b-modal ref="operate-examination-fmedical-item-modal" size="lg" centered title="修改处置" @ok="exitExaminationFmedicalItemsOk">
                      <b-row>
                        <b-col md="6">
                          <!--    药品编码 drugsCode-->
                          <b-form-group label="非药品编码" label-for="operateExaminationFmedicalItemsCode" :label-cols="3">
                            <b-form-input id="operateExaminationFmedicalItemsCode" required disabled v-model="operateExaminationFmedicalItemsForm.fmedicalItems.fmedicalItemsCode"
                              type="text" placeholder="请检索项目"></b-form-input>
                          </b-form-group>
                        </b-col>
                        <b-col md="6">
                          <!--    药品名称 drugsName-->
                          <b-form-group label="非药品名称" label-for="operateExaminationFmedicalItemsName" :label-cols="3">
                            <b-form-input id="operateExaminationFmedicalItemsName" required disabled v-model="operateExaminationFmedicalItemsForm.fmedicalItems.fmedicalItemsName"
                              type="text" placeholder="请检索项目"></b-form-input>
                          </b-form-group>
                        </b-col>
                      </b-row>
                      ...
                      <b-row>
                        <b-col md="6">
                          <!--    药品编码 drugsCode-->
                          <b-form-group label="非药品规格" label-for="operateExaminationFmedicalItemsFormat" :label-cols="3">
                            <b-form-input id="operateExaminationFmedicalItemsFormat" required disabled v-model="operateExaminationFmedicalItemsForm.fmedicalItems.fmedicalItemsFormat"
                              type="text" placeholder="请检索项目"></b-form-input>
                          </b-form-group>
                        </b-col>
                        <b-col md="6">
                          <!--    药品名称 drugsName-->
                          <b-form-group label="非药品价格" label-for="operateExaminationFmedicalItemsPrice" :label-cols="3">
                            <b-form-input id="operateExaminationFmedicalItemsPrice" required disabled v-model="operateExaminationFmedicalItemsForm.fmedicalItems.fmedicalItemsPrice"
                              type="text" placeholder="请检索项目"></b-form-input>
                          </b-form-group>
                        </b-col>
                      </b-row>
                      <b-row>
                        <b-col md="6">
                          <!--    药品编码 drugsCode-->
                          <b-form-group label="开立数量" label-for="operateExaminationQuantity" :label-cols="3">
                            <b-form-input id="operateExaminationQuantity" required v-model="operateExaminationFmedicalItemsForm.quantity"
                              type="number" placeholder="请添加数量"></b-form-input>
                          </b-form-group>
                        </b-col>
                        <b-col md="6">
                          <!--    药品编码 drugsCode-->
                          <b-form-group label="目的或要求" label-for="operateExaminationPurposeRequirements" :label-cols="3">
                            <b-form-input id="operateExaminationPurposeRequirements" required v-model="operateExaminationFmedicalItemsForm.purposeRequirements"
                              type="text" placeholder="请输入"></b-form-input>
                          </b-form-group>
                        </b-col>
                      </b-row>
                    ...
                    <br>
                    <b-table show-empty stacked="md" hover selectable select-mode="single" :items="examinationForm.examinationFmedicalItemsList"
                      :fields="examinationFmedicalItemsFields" @row-selected="selectExaminationFmedicalItems">
                    </b-table>
                  </b-col>
                </b-row>
              </b-card>
              <examination-drugs-table :hidden="!showAdditionDrugs" :examination-drugs-items-list="currentExaminationDrugsItemsList"
                @saveExaminationDrugsItems="saveExaminationDrugsItems">

              </examination-drugs-table>
            </b-tab>
            <b-tab :title="computedTitle2">
              <history-examination-list ref="history-examination-list" :type="type"></history-examination-list>
            </b-tab>
...
<script>
  ...
  export default {
    ...
    components: {
      ...
      PatientInfo,
      FmedicalTable,
      CommonlyUsedExamination,
      ExaminationDrugsTable,
    ...
    data() {
      return {
        ...
        commonlyUsedFields: [ //常用非药品条目的field
          {
            key: 'fmedicalItems.fmedicalItemsCode',
            label: '编码',
            sortable: true
          },
          {
            key: 'fmedicalItems.fmedicalItemsName',
            label: '名称',
            sortable: true
          },
          {
            key: 'fmedicalItems.fmedicalItemsFormat',
            label: '规格',
            sortable: true
          },
          {
            key: 'fmedicalItems.fmedicalItemsPrice',
            label: '价格',
            sortable: true
          },
        ],
    ...
    methods: {
      ...
      onSave() {
        ...
        for (let i = 0; i < this.examinationForm.examinationFmedicalItemsList.length; i++) {
          ...
          groupExaminationFmedicalItems.groupExaminationDrugsItemsList = this.examinationForm.examinationFmedicalItemsList[
            i].examinationDrugsItemsList;
        ...
        this.$post(this.api[this.type].insertGroupExaminationApi, JSON.parse(JSON.stringify(groupExamination))).then(
          res => {
            if (res.status === "OK") {
              alert(res.msg);
            } else {
      ...
      addExaminationFmedicalItem() { //新增非药品检查检验条目
        if (JSON.stringify(this.examinationFmedicalItemsForm.fmedicalItems) == "{}" || (!this.examinationFmedicalItemsForm
            .fmedicalItems.fmedicalItemsId)) { //判断是否有诊断信息
          alert("您还没有选择检查检验条目");
        } else {
      ...
      exitExaminationFmedicalItems() { //编辑非药品
        if (this.selectedExaminationFmedicalItemsIndex >= 0) {
          this.operateExaminationFmedicalItemsForm = Object.assign(this.examinationForm.examinationFmedicalItemsList[
            this.selectedExaminationFmedicalItemsIndex]);
      ...
      exitExaminationFmedicalItemsOk() { //确认编辑
        this.$set(this.examinationForm.examinationFmedicalItemsList, this.selectedExaminationFmedicalItemsIndex, Object
          .assign({}, this.operateExaminationFmedicalItemsForm));
      ...
      saveExaminationFmedicalItems() {
        if (this.selectedExaminationFmedicalItemsIndex >= 0) {
          ...
          commonlyUsedFmedical.fmedicalItemsId = this.examinationForm.examinationFmedicalItemsList[this.selectedExaminationFmedicalItemsIndex]
            .fmedicalItemsId;
          commonlyUsedFmedical.doctorId = this.curr_user.userId;
          this.$post(this.api[this.type].insertCommonlyUsedFmedicalApi, JSON.parse(JSON.stringify(commonlyUsedFmedical)))
            .then(res => {
              if (res.status === "OK") {
      ...
      additionExaminationFmedicalItems() { //加非药品
        if (this.selectedExaminationFmedicalItemsIndex >= 0) {
          ...
          this.currentExaminationDrugsItemsList = JSON.parse(JSON.stringify(this.examinationForm.examinationFmedicalItemsList[
            this.selectedExaminationFmedicalItemsIndex].examinationDrugsItemsList));
      ...
      saveExaminationDrugsItems(form) { //保存药品
        if (this.selectedExaminationFmedicalItemsIndex >= 0) {
          this.examinationForm.examinationFmedicalItemsList[this.selectedExaminationFmedicalItemsIndex].examinationDrugsItemsList =
            Object.assign([], form);
